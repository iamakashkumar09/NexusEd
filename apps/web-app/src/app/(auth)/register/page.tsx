...
function Field({ label, children, half, error }: { label: string; children: React.ReactNode; half?: boolean, error?: string }) {
  return (
    ...
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--ink-subtle)' }}>
          {step === 1 ? 'Join NexusEd — free forever for students' : `We've sent a 6-digit code to ${formData.email}`}
        </p>
      </div>
      ...
      {step === 1 ? (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
            {([
              ...
              return (
                <button
                  key={role} type="button" onClick={() => setFormData(prev => ({ ...prev, role }))}
                  style={{
          ...
          <form onSubmit={handleRegisterSubmit} autoComplete="off">
            <div style={{ display: 'flex', gap: 12 }}>
              <Field label="First name" half>
                <PremiumInput type="text" name="firstName" placeholder="Jane" value={formData.firstName} onChange={handleChange} autoComplete="off" />
              </Field>
              <Field label="Last name" half>
                <PremiumInput type="text" name="lastName" placeholder="Doe" value={formData.lastName || ''} onChange={handleChange} autoComplete="off" />
              </Field>
            </div>
            ...
            <Field label="Password">
              <PremiumInput type="password" name="password" placeholder="Min. 8 characters" value={formData.password} onChange={handleChange} autoComplete="new-password" />
            </Field>
            ...
            <button
              ...
              style={{
                ...
                background: loading ? 'var(--surface-3)' : 'var(--primary)', color: loading ? 'var(--ink-subtle)' : '#fff',
                fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                boxShadow: loading ? 'none' : '0 4px 20px rgba(94,106,210,0.35)', transition: 'all 0.15s',
              }}
            >
              {loading ? (
                <><span style={{ width: 16, height: 16, border: '2px solid var(--hairline-strong)', borderTopColor: 'var(--primary)', borderRadius: '50%', display: 'inline-block' }} className="animate-spin" /> Creating account...</>
              ) : 'Create Free Account →'}
            </button>
          </form>
        ...
        <form onSubmit={handleOtpSubmit} autoComplete="off">
          ...
          <button
            ...
            style={{
              width: '100%', padding: '12px 0', marginTop: 8,
              borderRadius: 'var(--r-md)', border: 'none', cursor: (loading || otp.length < 6) ? 'not-allowed' : 'pointer',
              background: (loading || otp.length < 6) ? 'var(--surface-3)' : 'var(--primary)', color: (loading || otp.length < 6) ? 'var(--ink-subtle)' : '#fff',
              fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              boxShadow: (loading || otp.length < 6) ? 'none' : '0 4px 20px rgba(94,106,210,0.35)', transition: 'all 0.15s',
            }}
          >
            {loading ? (
              <><span style={{ width: 16, height: 16, border: '2px solid var(--hairline-strong)', borderTopColor: 'var(--primary)', borderRadius: '50%', display: 'inline-block' }} className="animate-spin" /> Verifying...</>
            ) : 'Verify & Continue'}
          </button>
        </form>
      )}
